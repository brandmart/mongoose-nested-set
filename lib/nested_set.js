        ...
        if (!err && parentNode && parentNode.lft && parentNode.rgt) {
          ...
          self.siblings(function(err, nodes) {
            if (nodes.every(function(node) { return node.lft && node.rgt;})) {
              ...
              self.update({lft: { $gt: maxRgt}}, {$inc: {lft: 2}}, {multi: true}, function(err, updatedCount) {
                self.update({rgt: { $gt: maxRgt}}, {$inc: {rgt: 2}}, {multi: true}, function(err, updatedCount2) {
                  self.lft = maxRgt + 1;
                  self.rgt = maxRgt + 2;
                  next();
                });
...
//          console.log('WARNING: tree is not built for ' + modelName + ' nodes. Parent does not have lft/rgt');
  ...
  schema.method('parent', function(callback) {
    var self = this;
    self.findOne({_id: self.parentId}, callback);
  });
  ...
  schema.method('ancestors', function(callback) {
    ...
    self.selfAndAncestors(function(err, nodes) {
      if (err) {
        ...
        var nodesMinusSelf = nodes.filter(function(node) { return self._id != node._id });
  ...
  schema.method('children', function(callback) {
    var self = this;
    self.find({parentId: self._id}, callback);
  });
  ...
  schema.method('selfAndDescendants', function(callback) {
    var self = this;
    self.where('lft').gte(self.lft).where('rgt').lte(self.rgt).run(callback);
  });
  ...
  schema.method('selfAndSiblings', function(callback) {
    var self = this;
    self.find({parentId: self.parentId}, callback);
  });
