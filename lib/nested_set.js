...
//          console.log('WARNING: tree is not built for ' + modelName + ' nodes. Parent does not have lft/rgt');
  ...
  schema.method('ancestors', function(callback) {
    this.constructor.where('lft').lte(this.lft).where('rgt').gte(this.rgt).where('_id').ne(this._id).run(callback);
    // var self = this;
  ...
  schema.method('descendants', function(callback) {
    this.constructor.where('lft').gte(this.lft).where('rgt').lte(this.rgt).where('_id').ne(this._id).run(callback);
    // var self = this;
  ...
  schema.method('siblings', function(callback) {
    this.constructor.where('parentId', this.parentId).where('_id').ne(this._id).run(callback);
    // var self = this;
